...
DataFrame simulate_OPA(List params, double Tmax) {
  ...
  // Set time of first restocking event. Should be at the start of the simulation and should coincide with first scan.
  double next_restock_time = 0.0;
  ...
  while ((E + I != 0) && (tnow < Tmax) && (N > 0)) {
    ...
    // Determine next scan or restock time
    double tmanagement =
    std::min(next_scan_time, next_restock_time);

    // If scanning or restocking happens before another disease event
    if ((tmanagement < tnext) &&
        (tmanagement < tnew)) {

      tnow = tmanagement;

      bool do_scan =
        std::abs(tnow - next_scan_time) < 1e-8;

      bool do_restock =
        std::abs(tnow - next_restock_time) < 1e-8;
      ...
      if (do_restock) {
        ...
        next_restock_time += time_between_restocking;
      }
